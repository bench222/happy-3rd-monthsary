...
class Seed {
  ...
  drawText() {
    ...
    // Auto-shrink the font so longer text still fits neatly
    ...
    if (textWidth > maxTextWidth) {
      fontSize = Math.max(minFontSize, baseFontSize * (maxTextWidth / textWidth));
      ctx.font = `${fontSize}px sans-serif`;
      textWidth = ctx.measureText(text).width;
    }

    // Stretch the little callout line so it matches the text length
    const lineEndX = Math.max(80, 30 + textWidth + 10);

    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(20, 20);
    ctx.lineTo(lineEndX, 20);
    ctx.stroke();

    ctx.fillText(text, 30, 13);
    ctx.restore();
  }
